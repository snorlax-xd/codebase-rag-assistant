...
const criticalPaths = [
  {
    ...
    detail: "Sequence mapping reveals 4-hop traversal between Parser and VectorDb. Low latency detected.",
  },
  {
    ...
    <AppShell title="Architecture Graph" showRunAnalysis fullBleed>
      <div className="relative flex min-h-0 flex-1 grid-bg">
        ...
        <div className="pointer-events-none absolute inset-0 z-10">
                ...
                {criticalPaths.map((path) => (
                  <div key={path.name}>
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span
                        className={`shrink-0 rounded px-2 py-0.5 font-mono text-[10px] font-medium uppercase ${
                          path.status === "optimal"
          ...
          {/* Bottom-left: Vector Neighborhoods */}
          <div className="pointer-events-auto absolute bottom-6 left-6 w-56">
            <GlassPanel>
              ...
              <ul className="space-y-2">
                {clusters.map((c) => (
                  <li key={c.label} className="flex items-center gap-2 text-sm text-on-surface">
