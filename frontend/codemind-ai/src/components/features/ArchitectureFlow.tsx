...
export default function SemanticSearchPage() {
  ...
  useEffect(() => {
    ...
    const handleRepoChange = (e: Event) => {
      ...
      // Clear results when switching repos so stale data never shows
      setResults([]);
      setSearched(false);
      setQuery("");
    };
  ...
  return (
    <AppShell title="Semantic Search" fullBleed>
      <div className="flex min-h-0 flex-1 overflow-hidden">

        {/* ── Left: FileExplorer — has its own repo switcher built-in ── */}
        <FileExplorer
          onSelect={(file) => {
            setQuery(file);
            runSearch(file);
          ...
          <div className="mx-auto w-full max-w-4xl space-y-6">
              ...
              <Input
                ...
                placeholder={
                  activeRepo
                    ...
                    : "Select a repository from the explorer, then search..."
                }
            ...
            {!activeRepo && (
              <p className="font-mono text-xs text-on-surface-variant">
                ↑ Select a repository from the explorer on the left to enable search.
              </p>
            )}
